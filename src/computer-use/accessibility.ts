...
import { buildActionSpace, type ActionSpace } from '../core/action-space.js';
import { classifyRisk } from '../core/policy.js';
import { redactElement } from '../core/redaction.js';
import type { ElementNode, PageSnapshot, Operation } from '../core/types.js';
...
function findRole(line: string): { spec: RoleSpec; marker: string; index: number } | undefined {
  const lowered = line.toLocaleLowerCase();
  let best: { spec: RoleSpec; marker: string; index: number } | undefined;
...
function labelFromLine(line: string, role: { marker: string; index: number }): string {
...
function operationsFor(spec: RoleSpec, line: string): Operation[] {
...
function nodeFromLine(index: string, line: string, role: { spec: RoleSpec; marker: string; index: number }): ElementNode | undefined {
  if (isDisabled(line)) return undefined;
...
export function normalizeAccessibilityState(state: SkyAppState): PageSnapshot {
  ...
  for (const line of rawText.split(/\r?\n/)) {
    ...
    if (!index || !content || seen.has(index)) continue;
...
export function buildComputerUseActionSpace(snapshot: PageSnapshot): ActionSpace {
