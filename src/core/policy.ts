...
export function classifyRisk(input: { role?: string; name?: string }): Risk {
  const label = `${input.role ?? ''} ${input.name ?? ''}`;
  if (irreversiblePattern.test(label)) return 'irreversible';
  if (sensitivePattern.test(label)) return 'sensitive';
  return 'none';
}

export function canExecute(operation: Operation, risk: Risk, approved = false): { allowed: true } | { allowed: false; reason: string } {
